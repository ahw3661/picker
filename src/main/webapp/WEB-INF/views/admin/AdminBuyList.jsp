		...
		<div class="buy_list_wrap">
							...
							<c:forEach var="item" items="${buyItem }">
								<c:if test="${list.b_code == item.b_code }">
									<tr>
										...
										<td class="code_td">
											<a href="javascript:buyDetail('${item.b_code }', ${pgdto.pageNum }, '${start_date }', '${end_date }');">${item.b_code }</a>
			...
			<div class="centerBlock">
	 			<c:if test="${pgdto.startPage > 1}">
	 				<div class="prev_div"><a href="javascript:allBuyList(${pgdto.startPage - pgdto.pageSize}, '${start_date }', '${end_date }');"><b>《</b></a></div>
	 			</c:if>
				<c:forEach var="page" begin="${pgdto.startPage}" end="${pgdto.endPage}">
					<c:if test="${page >= 1}">
						<c:if test="${page != pgdto.pageNum}">
							<div class="page_div"><a href="javascript:allBuyList(${page}, '${start_date }', '${end_date }')">${page}</a></div>
						</c:if>
						<c:if test="${page == pgdto.pageNum}">
							<div class="curr_div"><a href="javascript:allBuyList(${page}, '${start_date }', '${end_date }')">${page}</a></div>
						</c:if>
					</c:if>
				</c:forEach>
	 			<c:if test="${pgdto.endPage < pgdto.pageCount}">
	 				<div class="next_div"><a href="javascript:allBuyList(${pgdto.startPage + pgdto.pageSize }, '${start_date }', '${end_date }');"><b>》</b></a></div>
	 			</c:if>
			</div>
		</div>
	</section>	
...
<script type="text/javascript">
	...
	// 주문조회상세
	function buyDetail(cd, pn, sd, ed) {
		$.ajax({
			url : "buyDetail",
			type : "post",
