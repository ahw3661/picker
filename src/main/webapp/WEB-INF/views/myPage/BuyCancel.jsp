		...
		<div class="buy_cancel_wrap">
			...
			<div class="choice_input">
				...
				<div class="date_search_div">
					<input type="hidden" name="startDate" value="${start_date }" id="start"><input type="hidden" name="endDate" value="${end_date }" id="end">
			...
			<div class="count_div">총 ${cnt }건<input type="hidden" name="cnt" value="${cnt }" id="count"></div>
								...
								<c:forEach var="item" items="${buyitem }">
									<c:if test="${list.b_code == item.b_code }">
										<tr>
											...
											<td class="code_td"><a href="javascript:buyCancelDetail('${item.b_code }', ${pgdto.pageNum }, '${start_date }', '${end_date }');">${item.b_code }</a></td>
											<td class="item_td">
												<img alt="img" src="resources/image/category_img/${item.i_img }">&nbsp;
												<a href="goDetail?i_code=${item.i_code}">${item.i_name }</a>
											</td>
			...
			<c:if test="${pageNum > 0 }">
				<div class="centerBlock">
		 			<c:if test="${pgdto.startPage > 1}">
		 				<div class="prev_div"><a href="javascript:buyCancel(${pgdto.startPage - pgdto.pageSize}, '${start_date }', '${end_date }');"><b>《</b></a></div>
		 			</c:if>
					<c:forEach var="page" begin="${pgdto.startPage}" end="${pgdto.endPage}">
						<c:if test="${page >= 1}">
							<c:if test="${page != pgdto.pageNum}">
								<div class="page_div"><a href="javascript:buyCancel(${page}, '${start_date }', '${end_date }')">${page}</a></div>
							</c:if>
							<c:if test="${page == pgdto.pageNum}">
								<div class="curr_div"><a href="javascript:buyCancel(${page}, '${start_date }', '${end_date }')">${page}</a></div>
							</c:if>
						</c:if>
					</c:forEach>
		 			<c:if test="${pgdto.endPage < pgdto.pageCount}">
		 				<div class="next_div"><a href="javascript:buyCancel(${pgdto.startPage + pgdto.pageSize }, '${start_date }', '${end_date }');"><b>》</b></a></div>
		 			</c:if>
				</div>
			</c:if>
		</div>
...
<script type="text/javascript">
	...
	$("#dsBtn").click(function() {
		$.ajax({
			url : "buyCancel",
			type : "post",
			data : { "pageNum" : $("#pagenum").val(), "start_date" : $("#from").val(), "end_date" : $("#to").val() },
			datatype : "html",
			success : function(data) {
				$(".menu_info").children().remove();
				$(".menu_info").html(data);
	...
	// 주문조회상세
	function buyCancelDetail(cd, pn, sd, ed) {
		$.ajax({
			url : "buyCancelDetail",
			type : "post",
